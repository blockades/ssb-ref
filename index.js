...
var ip = require('ip');
var protocolRegex = /^(net|wss?|onion|dht)$/;
...
var isLink = (exports.isLink = function(data) {
  return isString(data) && linkRegex.test(data);
});

var isFeedId = (exports.isFeed = exports.isFeedId = function(data) {
  return isString(data) && feedIdRegex.test(data);
});

var isMsgId = (exports.isMsg = exports.isMsgId = function(data) {
  return isString(data) && msgIdRegex.test(data);
});

var isBlobId = (exports.isBlob = exports.isBlobId = function(data) {
  return isString(data) && blobIdRegex.test(data);
});

var normalizeChannel = (exports.normalizeChannel = function(data) {
...
var isLegacyInvite = (exports.isLegacyInvite = function(data) {
  if (!isString(data)) return false;
  data = data.replace(legacyInviteFixerRegex, '');
  var parts = data.split('~');
  return (
    parts.length == 2 && isAddress(parts[0]) && legacyInviteRegex.test(parts[1])
  );
});

var isMultiServerInvite = (exports.isMultiServerInvite = function(data) {
  if (!isString(data)) return false;
  return !!parseMultiServerInvite(data);
});

var isInvite = (exports.isInvite = function(data) {
  if (!isString(data)) return false;
...
function parseLegacyInvite(invite) {
  ...
  var protocol = 'net:';
  if (addr.host.endsWith('.onion')) protocol = 'onion:';
  var remote =
    protocol + addr.host + ':' + addr.port + '~shs:' + addr.key.slice(1, -8);
...
function parseMultiServerInvite(invite) {
  ...
  var parts = invite.split('~').map(function(e) {
    return e.split(':');
  });
...
exports.parseInvite = function(invite) {
  return isLegacyInvite(invite)
    ...
    : isMultiServerInvite(invite) ? parseMultiServerInvite(invite) : null;
};
...
exports.type = function(id) {
  ...
  else if (c == '%' && isMsgId(id)) return 'msg';
  else if (c == '&' && isBlobId(id)) return 'blob';
